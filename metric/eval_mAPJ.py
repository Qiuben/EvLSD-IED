...
def eval_mAPJ(gt_file, pred_file):
    ...
    for i, (gt_ann, pred_ann) in enumerate(zip(gt_annotations, pred_annotations)):
        ...
        # 替换为 下面三句
        # junc_pred, junc_score = pred_ann['juncs_pred'], pred_ann['juncs_score']
        # junc_pred = np.array(junc_pred)
        # junc_score = np.array(junc_score)
        # print(colored(junc_pred.shape, 'red'))
...
if __name__ == '__main__':
    ...
    pred_file = '/home/lihao/workspace/python/self/Event-Enhanced-Frame-Line-Segment-Detection/homo_event_HAWP/output/hawpv2_pred.json'
