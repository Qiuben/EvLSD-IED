...
from network.build import build_hg
from network.hafm import HAFMencoder
...
class WireframeDetector(nn.Module):
    def __init__(self, cfg, backbone_in_channels):
        super().__init__()
        ...
        self.in_channels = backbone_in_channels
        self.backbone = build_hg(cfg, self.in_channels)
    ...
    def pooling(self, loi_feature, lines):
        ...
        logits = self.fc(xp).flatten() # self.fc 就是我们的验证头
        return logits
    ...
    def forward_test(self, images, annotations):
        ...
        for i in range(B):
            ...
            if self.use_residual:
                line_preds = self.proposal_lines(md_pred, dis_pred, res_pred, self.scale).view(-1, 4)  # 将 afm 转化成线
            else:
                line_preds = self.proposal_lines(md_pred, dis_pred, None, self.scale).view(-1, 4)

            jloc_pred_nms = non_maximum_suppression(jloc_pred)  #1, 128, 128  结合点预测分支
            topK = min(self.n_dyn_junc, int((jloc_pred_nms > self.junc_thresh).float().sum().item()))
            junc_preds = get_junctions(jloc_pred_nms, joff_pred, topk=topK)  # N,2
    ...
    def forward_train(self, images, annotations):
        ...
        mask = targets['mask']   # bs, 1, 128, 128
        ...
        for i in range(batch_size):
            ...
            # print('md_pred shape:', md_pred.shape)  #3,128,128
            # print('dis_pred shape:', dis_pred.shape)  #1,128,128
            # print('res_pred shape:', res_pred.shape)
            # print('jloc_pred shape:', jloc_pred.shape)
            # print('joff_pred shape:', joff_pred.shape) #2,128,128
            ...
            junc_preds = get_junctions(non_maximum_suppression(jloc_pred), joff_pred, topk=min(N * 2 + 2, self.n_dyn_junc)) #map to coordinate
            
            # print(colored('line_preds:{}'.format(line_preds.shape), 'red'))
            # print(colored('junc_preds:{}'.format(junc_preds.shape), 'blue'))
            # ERROR 
            dis_junc_to_end1, idx_junc_to_end1 = torch.sum((line_preds[:, :2] - junc_preds[:, None]) ** 2, dim=-1).min(0) #返回这300个点和预测的线的端点距离的最小值和对应的index
            dis_junc_to_end2, idx_junc_to_end2 = torch.sum((line_preds[:, 2:] -   junc_preds[:, None]) ** 2, dim=-1).min(0)

            # print('dis_junc_to_end1:', dis_junc_to_end1.shape)
            # print('idx_junc_to_end1:', idx_junc_to_end1.shape)
