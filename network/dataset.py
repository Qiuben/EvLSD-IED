...
class Resize(object):
    def __init__(self, image_size, ann_size): # heatmap_size: [128, 128]
...
class Dataset(Data.Dataset):
    ...
    def __init__(self, cfg, split, mode,  dataset):
        ...
        image_folder = 'images'
        # event_folder = 'e-wireframe/events-EST-16'
        # event_folder = 'e-wireframe/events-HG-2'
        event_folder = 'events-EST-10'
           
        self.image_file_list = [os.path.join(root, image_folder, self.annotations[i]['filename'])
                                for i in range(len(self.annotations))]
        self.event_file_list = [os.path.join(root, event_folder, self.annotations[i]['filename'].replace('.png', '.npz'))
                                for i in range(len(self.annotations))]

    def __getitem__(self, index):  
        if (self.mode == 'val') or (self.mode == 'export'):
            ...
            width = ann['width']
            height = ann['height']
            im = Image.open(self.image_file_list[index])
            image = np.asarray(im.convert('RGB')) # right
            
            # 可视化
            # img_vis = image.copy()
            # wandb.log({'image': wandb.Image(img_vis)})
            ...
            elif reminder == 4:   # 逆时针转90
                image = np.rot90(image)
                event = np.rot90(event)
                ori_x = copy.deepcopy(ann['junc'][:, 0])
                ori_y = copy.deepcopy(ann['junc'][:, 1])
                ann['junc'][:, 0] = ori_y  * (width/height) 
                ann['junc'][:, 1] = (width - ori_x) * (height/width) 

            
            elif reminder == 5:  #顺时针转90
                image = np.rot90(image, k=-1)
                event = np.rot90(event, k=-1)

                ori_x = copy.deepcopy(ann['junc'][:, 0])
                ori_y = copy.deepcopy(ann['junc'][:, 1])
                
                ann['junc'][:, 0] = (height - ori_y) *(width/height) 
                ann['junc'][:, 1] = ori_x * (height/width) 

                           
            else:  # 不做变换
                image = image
                event = event
                ann['junc'][:, 0] = ann['junc'][:, 0]
                ann['junc'][:, 1] = ann['junc'][:, 1]
            ...
            ann['junc'][:, 0] = np.clip(ann['junc'][:, 0] * sx, 0, 128 - 1e-4)
            ann['junc'][:, 1] = np.clip(ann['junc'][:, 1] * sy, 0, 128 - 1e-4)
    ...
    def transforms(self, image, event, ann=None):
        if (self.mode == 'val' or self.mode == 'export') : 
            ...
            transforms = Compose([
                Resize(self.image_size, self.heatmap_size),   # heatmap_size: [128, 128]
                ToTensor(),
                Normalize(self.mean, self.std)
            ])
    ...
    @staticmethod
    def collate(batch):
        return default_collate([b[0] for b in batch]), default_collate([b[1] for b in batch]), [b[2] for b in batch]
